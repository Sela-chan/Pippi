...
║ ${osuE.hit300}: \`${info[i].counts['300']}\` ║ ${osuE.hit100}: \`${info[i].counts['100']}\` ║ ${osuE.hit50}: \`${info[i].counts['50']}\` ║ ${osuE.hit0}: \`${info[i].counts.miss}\` ║\n`;
         ...
         const embed = new RichEmbed()
            .setAuthor(`Jugadas recientes en osu!STD de ${bd.get('user').username}`, `https://osu.ppy.sh/images/flags/${bd.get('user').country.toUpperCase()}.png`, `https://osu.ppy.sh/users/${bd.get('user').user_id}/osu`)
